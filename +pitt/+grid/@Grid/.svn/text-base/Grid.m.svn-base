classdef Grid < handle
    ...
    methods( Access=public )
        ...
        function setExecutable( obj, exec )
            exec = [ exec, ';quit;' ];
            exec = [ 'cd ', obj.getJobPath(), '\n', exec ];
            %exec = strrep( exec, '"', '\"' );
            obj.jobExec = exec;
            obj.generateQsubFile();
        end
        
        function executePBS( obj )
            syscmd = obj.qsubExec;
            syscmd = [syscmd, ' -wd ', obj.getJobPath(), ' -N ', obj.getThisUser,'-',obj.getJobName(), ' ', obj.getPBSFile() ];
            disp( ['Executing: ', syscmd ]);
            obj.runSysCmd( syscmd );
        end
    ...
    methods( Access=private )
        ...
        function init = initQsub( obj )
            init = '#!/bin/sh\n';
            init = [ init, 'echo "Executing @: `date` on host: `hostname` by user: `whoami` "\n']; 
        end
    ...
    methods (Static)
        function runSysCmd( cmdString )
            system( cmdString );
        end
        
        % Generate a unique log location
        function newPath = makeLogDirectory( jobName )
            newPath = pitt.grid.Grid.getLogDirectory( jobName );
        ...
        function newPath = getLogDirectory( jobName )
            newPath = [pitt.grid.Grid.logLocation, pitt.grid.Grid.getThisUser(), '/', jobName, '/'];
        end
