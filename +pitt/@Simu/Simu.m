classdef Simu < handle % pitt.Simu
    ...
    methods
        ...
        % Outputs two files with 1* and 2* prepended to them. i.e. 1subj-sim.fif
        % output fif file is in sensor space
        %{
        
        /synapse/home/schmidtb/mne/bin/mne_simu --fwd fwd.fif --label LS1-lh.label --meg --out 1subj-sim.fif --sfreq 150 --tmin 0 --tmax 600000 --senscov cov.fif --nave 4 --raw --timecourse sinewave_timecourse1.txt
        ...
        /synapse/home/schmidtb/mne/bin/mne_process_raw --raw subj-sim.fif --filteroff --ave fake_ave.ave --saveavetag subj-sim.fif_ave
        /synapse/home/schmidtb/mne/bin/mne_make_movie --inv /synapse/home/schmidtb/simu/base/inv.fif --meas subj-simsubj-sim.fif_ave.fif --signed --set 1 --picknormalcomp --subject subj --stc subj-sim.fif.stc
        %}
    ...
    methods( Static )
        ...
        function sim = avniel_clone_empty_room()
            tic; 
            pitt.Depend.MNEadd();
            sim = pitt.Simu( 'subj', '~/simu/avniel_clone_empty_room', 'LS1-lh.label', 'RS1-rh.label', 'sinewave_timecourse1.txt', 'sinewave_timecourse2.txt' );
            sim.numaverages = 1;
            sim.simulate();
            toc;
        end
        ...
        function sim = plv_lh_dlpf_rh_s1()
            tic; 
            pitt.Depend.MNEadd();
            sim = pitt.Simu( 'subj', '~/simu/plv_lh_dlpf_rh_s1', 'DLPF-lh.label', 'RS1-rh.label', 'sinewave_timecourse1.txt', 'sinewave_timecourse2.txt' );
            sim.numaverages = 4;
            sim.simulate();
            toc;
        end
        
        function sim = plv_opt001_dual_s1()
            tic; 
            pitt.Depend.MNEadd();
            sim = pitt.Simu( 'subj', '~/simu/opt001_s1_sim', 'LS1-lh.label', 'RS1-rh.label', 'sinewave_timecourse1.txt', 'sinewave_timecourse2.txt' );
            sim.numaverages = 4;
            sim.simulate();
            toc;
        end
        
        function sim = plv_opt001_empty()
            tic; 
            pitt.Depend.MNEadd();
            sim = pitt.Simu( 'subj', '~/simu/opt001_empty_sim', 'LS1-lh.label', 'RS1-rh.label', 'sinewave_timecourse1.txt', 'sinewave_timecourse2.txt' );
            sim.numaverages = 4;
            sim.simulate();
            toc;
        end
