...
% Function h=pitt.grtheory.grPlot(V,E,kind,vkind,ekind,sa) 
% draw the plot of the pitt.grtheory.graph (dipitt.grtheory.graph).
% Input parameters: 
...
%   E(m,2) or (m,3) - the edges of pitt.grtheory.graph (arrows of dipitt.grtheory.graph)
...
%     For disconnected pitt.grtheory.graph use E=[] or h=Plotpitt.grtheory.graph(V).
%   kind - the kind of pitt.grtheory.graph.
%   kind = 'g' (to draw the pitt.grtheory.graph) or 'd' (to draw dipitt.grtheory.graph);
%   (optional, 'g' default).
...
if ~isempty(V),
  ...
  if nargin==1, % disconnected pitt.grtheory.graph
    E=[];
  end
end
if ~isempty(E), % for connected pitt.grtheory.graph
  [m,n,newE]=pitt.grtheory.grValidation(E);
...
% we paint the pitt.grtheory.graph
hh=figure;
hold on
...
if ~isempty(E),
  ...
  while k<m2,
    ...
    for k2=1:k1, % we draw the edges (arrows)
      ...
      plot(MyXg,MyYg,'k-');
      if kind1=='d', % dipitt.grtheory.graph with arrows
        if E2(k+k2-1,1)==1,
